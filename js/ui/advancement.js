(function(){
  ...
  const teamLine=(team,mark='',origin)=>`<span class="advance-team ${mark}"><span class="advance-team-heading">${teamLogoMarkup(team,'advance')}<span class="advance-team-name"><strong>${zh(team)}</strong><small>${canonicalTeamName(team)}</small>${origin?`<i class="team-origin ${origin.className}">${origin.label}</i>`:''}</span></span></span>`;
