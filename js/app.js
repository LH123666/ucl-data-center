...
document.documentElement.dataset.staticVerified=window.uclSeason.current?'2026-08-26':window.uclSeason.end;
...
const rankHistoryMarkup=(team,currentRank)=>{if(window.uclSeason.previousLabel==='2023/24')return '<span class="previous-rank" title="2023/24为分组赛制，无法直接比较统一排名">上季小组赛制</span>';const previous=team[12],change=leagueRankChange(previous,currentRank,leaguePhaseStarted());if(change.kind==='new')return `<span class="newcomer-knight" title="${window.uclSeason.previousLabel}无可比较的欧冠联赛阶段排名" aria-label="上赛季无可比较排名">♞</span>`;const previousBadge=`<span class="previous-rank" title="${window.uclSeason.previousLabel}欧冠联赛阶段最终排名">上季 ${previous}</span>`;if(change.kind==='pending')return previousBadge;const arrow=change.kind==='up'?'↑':change.kind==='down'?'↓':'→';return `${previousBadge}<span class="rank-change ${change.kind}" title="相较上赛季联赛阶段排名${change.kind==='up'?'上升':change.kind==='down'?'下降':'不变'}${change.value}位">${arrow}${change.value}</span>`};
function render(q=''){tbody.innerHTML=teams.map((t,i)=>({t,i})).filter(({t})=>(t[0]+t[1]).toLowerCase().includes(q.toLowerCase())).map(({t,i})=>`<tr class="rank r${i+1} pot-${t[11]}" data-team="${t[1]}" tabindex="0" aria-label="查看${t[0]}详情"><td>${i+1}</td><td class="team">${teamLogoMarkup(t[1],'standings')}<span class="bilingual"><strong>${t[0]}</strong><small>${t[1]}</small></span><span class="team-history">${rankHistoryMarkup(t,i+1)}</span><em class="pot-chip">第${t[11]}档</em></td><td>${t[3]}</td><td>${t[4]}</td><td>${t[5]}</td><td>${t[6]}</td><td>${t[7]}–${t[8]}</td><td>${t[7]-t[8]>0?'+':''}${t[7]-t[8]}</td><td class="pts">${t[9]}</td><td><span class="form">${[...t[10]].map(x=>`<i class="${x.toLowerCase()}">${x==='W'?'胜':x==='D'?'平':'负'}</i>`).join('')||'—'}</span></td></tr>`).join('');}
