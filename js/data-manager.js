(function(){
  ...
  const renderSchedule=()=>{
    ...
    grid.innerHTML=Object.entries(days).slice(0,9).map(([date,fs])=>`<article class="fixture-day"><div class="fixture-date"><span>${date.slice(5).replace('-','月')}日</span><b>${['日','一','二','三','四','五','六'][new Date(date+'T12:00:00').getDay()]}</b></div>${fs.map(f=>`<div class="fixture" data-date="${f.date}" data-time="${f.time}" data-home="${encodeURIComponent(f.home)}" data-away="${encodeURIComponent(f.away)}"><time>${f.time} · 北京时间 · 欧冠</time><div class="fixture-teams">${fixtureClubMarkup(f.home)}<i>VS</i>${fixtureClubMarkup(f.away)}</div>${predictionSummary(f)}<small class="fixture-more">查看比赛详情 →</small></div>`).join('')}</article>`).join('');
  ...
  const historySeasons=['2023/24','2024/25','2025/26'];
  ...
  const renderSideStandings=(home,away)=>{
    ...
    const column=(rows,offset)=>`<section class="side-column" aria-label="第${offset+1}至${offset+rows.length}名"><div class="side-table-head"><span>#</span><span>球队</span><span>赛</span><span>净胜</span><span>积分</span></div><div class="side-table-body">${rows.map((t,i)=>`<div class="side-team pot-${t[11]} ${t[1]===home||t[1]===away?'selected':''} zone-${offset+i+1}"><b>${offset+i+1}</b><span><strong>${escapeHtml(t[0])}</strong><small title="${escapeHtml(t[1])}">${escapeHtml(t[1])}</small></span><i>${t[3]}</i><i>${t[7]-t[8]>0?'+':''}${t[7]-t[8]}</i><em>${t[9]}</em></div>`).join('')}</div></section>`;
    panel.innerHTML=`<div class="side-table-title"><div><p>UEFA CHAMPIONS LEAGUE · 2026/27</p><h2>联赛阶段积分榜</h2></div><span>当前 ${teams.length} 队</span><button type="button" class="standings-return">返回比赛</button></div><div class="side-columns" tabindex="0" aria-label="积分榜，可滚动查看全部球队">${column(teams.slice(0,mid),0)}${column(teams.slice(mid),mid)}</div><div class="side-legend"><span><i></i>1–8直通16强</span><span><i></i>9–24附加赛</span><span><i></i>25–36淘汰</span></div>`;
