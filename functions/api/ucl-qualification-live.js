...
function parseOfficial(text){
  ...
  for(const tie of PLAYOFF_TIES){
    for(const [home,away] of [tie,[tie[1],tie[0]]]){
      ...
      const result=plain.match(new RegExp(`(?:${homeNames})\\s+(\\d+)\\s*[-–]\\s*(\\d+)\\s+(?:${awayNames})`,'i'));
      if(!result)continue;
...
export async function onRequestGet(){
  ...
  let official=[],error='';
  for(const [url,label] of [[OFFICIAL_URL,'UEFA'],[`https://r.jina.ai/http://www.uefa.com/uefachampionsleague/accesslist/?refresh=${Math.floor(Date.now()/300000)}`,'UEFA proxy']]){
    try{
      ...
      official=parseOfficial(body);if(official.length)break;
      throw new Error(`${label} returned no playoff results`);
